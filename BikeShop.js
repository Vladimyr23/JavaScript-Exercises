...
if (time >= 7) {
    TotalRent=Total(DailyRent, time);
    TotalRent = Total - Total * 0.25 + 10;
    alert("Rent cost including £10 deposit and 25% discount: " + TotalRent);
} else if (confirm("Are you a Cycling club member?")) {
    Total(DailyRent, time);
    TotalRent = TotalRent - TotalRent * 0.15 + 10;
    alert("Rent cost including £10 deposit and 15% discount: " + TotalRent);
} else if (confirm("Are you an old age pensioner?")) {
    TotalRent=Total(DailyRent, time);
    TotalRent = TotalRent - TotalRent * 0.15 + 10;
    alert("Rent cost including £10 deposit and 15% discount: " + TotalRent);
} else if (confirm("Are you a student?")) {
    TotalRent = DailyRent * time;
